...
# 原参数: loc=0.88, scale=0.05
# 新参数: 降低了均值，增大了标准差，使其与边界样本重叠更多
same_person = np.random.normal(loc=0.82, scale=0.08, size=10000)
same_person = np.clip(same_person, 0, 1)

# 原参数: loc=0.78, scale=0.09
# 新参数: 均值保持不变，但增大了标准差，使其同时与两边都有更多重叠
boundary = np.random.normal(loc=0.76, scale=0.12, size=10000)
boundary = np.clip(boundary, 0, 1)

# 原参数: loc=0.35, scale=0.13
# 新参数: 提高了均值，增大了标准差，使其尾部能延伸到边界区域
different = np.random.normal(loc=0.45, scale=0.18, size=10000)
...
# 填充曲线（从下往上）
ax.fill_between(x_grid, 0, y_diff, color='#ff9f40', alpha=0.9, label='不同人 (均值0.45)')
ax.fill_between(x_grid, 0, y_boundary, color='#66c2a5', alpha=0.9, label='边界样本 (均值0.76)')
ax.fill_between(x_grid, 0, y_same, color='#5ab4ac', alpha=0.9, label='同一个人 (均值0.82)')
...
ax.set_xlim(0, 1)
ax.set_ylim(0, max(y_same.max(), y_boundary.max(), y_diff.max()) * 1.2)  # 动态调整y轴上限
...
legend_elements = [
    ...
    mpatches.Patch(color='#5ab4ac', label='同一个人 (均值0.82)'),
    mpatches.Patch(color='#66c2a5', label='边界样本 (均值0.76)'),
    mpatches.Patch(color='#ff9f40', label='不同人 (均值0.45)'),
